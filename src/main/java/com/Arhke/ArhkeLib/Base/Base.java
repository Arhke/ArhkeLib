...
import com.Arhke.ArhkeLib.ItemUtil.ItemUtil;
...
//        message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("Visit the Spigot website!").create()));
    ...
    public static void customDamage(LivingEntity attacker, LivingEntity target, double damage){
        if(damage <= 0) return;
        if(target.isDead()) return;
        damage = Math.min(target.getHealth(), damage);
        TrueDamageEvent event;
...
//        return !WGBukkit.getRegionManager(player.getWorld())
...
//    }
    ...
    public static void highlightItem(ItemStack is){
        ItemUtil.addEnchant(is, Enchantment.ARROW_INFINITE, 1, true);
    }
